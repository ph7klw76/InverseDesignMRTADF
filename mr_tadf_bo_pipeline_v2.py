...
def _worker_generate(args):
    ...
    Communicates via shared counter + per-worker local set.
    Returns a list of (smi, mol) found by this worker.
    """
    (worker_id, seed_smiles_list, n_target_global,
     attempts_per_worker, global_counter, lock, global_seen_set_proxy) = args

    # Parse seeds inside this process (avoids pickling Mol objects)
    seed_mols = []
    ...
    for attempt in range(attempts_per_worker):
        # Early exit if global target reached
        if attempt % 500 == 0:
            with lock:
                if global_counter.value >= n_target_global:
                    break
        ...
        try:
            ...
            if passes_mr_tadf_filter(new_mol):
                ...
                local_found.append((new_smi, new_mol))
                with lock:
...
def generate_mr_tadf_candidates_parallel(
    ...
    # Also generate via BRICS recombination
    log.info("  Building BRICS fragment library ...")
    ...
    for smi in seed_smiles:
        ...
        if mol is not None:
            try:
                ...
                pass
    log.info("  BRICS fragment library: %d unique fragments", len(all_frags))

    # Shared state for workers
    counter = Value('i', len(candidates_list))
    lock = Lock()

    # We pass SMILES strings, not Mol objects (they can't be pickled reliably)
    worker_args = [
        (wid, seed_smiles, n_target, attempts_per_worker, counter, lock, None)
